...
const JS_STUBS = ['@yydb/iris', '@yydb/iris-skills'];

const NATIVE_STUBS = [
    { name: '@yydb/iris-win32-x64', os: ['win32'], cpu: ['x64'] },
    { name: '@yydb/iris-linux-x64', os: ['linux'], cpu: ['x64'] },
    { name: '@yydb/iris-unknown-wasm32', description: 'Iris ORM browser WASM placeholder — not for production use.' },
];
...
/** @type {StubSpec[]} */
const STUBS = [...JS_STUBS.map((name) => ({ name })), ...NATIVE_STUBS];
...
function runNpm(args, opts = {}) {
    ...
    let userConfig;
    if (token) {
        userConfig = path.join(os.tmpdir(), `iris-placeholder-npmrc-${process.pid}`);
...
function writeStub(spec, dir, authToken) {
    ...
    const pkg = {
        name: spec.name,
        version: VERSION,
        description: spec.description ?? 'Iris ORM placeholder — not for production use.',
        license: 'MIT',
        private: false,
        files: ['README.md'],
    };
    ...
    fs.writeFileSync(path.join(dir, 'README.md'), `# ${spec.name}\n\nPlaceholder package (${VERSION}). Reserved for the Iris ORM project.\n`);
    if (authToken) {
...
function cmdPublish() {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'iris-npm-stubs-'));
